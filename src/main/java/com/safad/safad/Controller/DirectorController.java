...
public class DirectorController
{
    ...
    @GetMapping("/Index_Director")

    public String Directors()
    {
        return "Directors/Index_Director";
    }
    ...
    public String crear_Role(Model model)
    {
        ...
        model.addAttribute("titulo", "FORMULARIO DE ROLES");
        model.addAttribute("role", role);

        return "Directors/Form_Role";
    }
    ...
    @GetMapping("/Eliminar_Role/{Id_Role}")

    public String Eliminar_Role(@PathVariable Long Id_Role)
    {
        if(Id_Role > 0)
        {
            Aux1.deleteById(Id_Role);
        }
    ...
    public String buscar(@RequestParam(name="Name_Rol", required=false) String Name_Rol, Model model)
    {
        if (Name_Rol != null && !Name_Rol.isEmpty())
        {
            List<People> people = peopleDetails.findByRole(Name_Rol);
